...
abstract class JellyfinWebViewClient(
    ...
    override fun onPageFinished(view: WebView, url: String?) {
        ...
        // Safety-net: ensure portalfin-restyle.js runs on every page, even
        // ones that don't load main.jellyfin.bundle.js (e.g. server-select,
        // post-logout login). Read script from assets and evaluateJavascript.
        try {
            view.context.assets.open("native/portalfin-restyle.js").use { stream ->
                val script = stream.bufferedReader().readText()
                view.evaluateJavascript(script, null)
            }
        } catch (e: Throwable) {
            Timber.w(e, "Failed to inject portalfin-restyle.js")
        }
    }
